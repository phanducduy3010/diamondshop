...
<div class="well well-small">
    ...
    <c:if test="${ allProductsPagination.size() > 0 }">
    <div class="row-fluid">
        <ul class="thumbnails">
            <c:forEach var="item" items="${ allProductsPagination }" varStatus="loop">
            <li class="span4">
                <div class="thumbnail">
            ...
            <c:if test="${ (loop.index + 1) % 3 == 0 || (loop.index + 1)  == allProductsPagination.size() }">
        </ul>
    </div>
    <c:if test="${ (loop.index + 1) < allProductsPagination.size() }">
    <div class="row-fluid">
        <ul class="thumbnails">
            </c:if>
            </c:if>
    ...
    <div class="pagination">
        <c:forEach var="item" begin="1" end="${paginationInfo.totalPages}" varStatus="loop">
            <c:if test="${ (loop.index) == paginationInfo.currentPage}">
